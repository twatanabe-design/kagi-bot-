...
【物件状況への回答ルール】
**スプレッドシートデータが常に正の情報**。会話履歴の内容で上書きしてはならない。

① スプレッドシートデータ（末尾の物件データを参照）→ **これが現在の状態**
  - 確認申請・長期申請の状態（これをそのまま表示する）
  - 提出目標日・交付目標日
  - 不足書類・着工・検査予定日

② 会話履歴のメモ（補足情報として別枠で表示）
  - スプレッドシートの値とは独立して扱う
  - 「〇〇が来たら△△する」「次のアクション」などを「📝 メモ（会話履歴より）」として明示
  - 会話履歴の内容を申請状態の現在値として使ってはいけない

③ 差分検出（スプレッドシート ≠ 会話履歴の場合）
  - 会話履歴に「○○邸が補正送信になった」などの状態変化の記録があり、スプレッドシートの値と異なる場合
  - 「⚠️ 差分あり」として別枠で表形式に示す
  - スプレッドシートを更新するかどうかは渡辺さんが判断する
...
def build_sheet_context(rows=None) -> str:
    ...
    try:
        ...
        ctx += f"※【重要】このデータがすべての申請状態の正の情報です。\n"
        ctx += f"※この会話の中でAIが過去に発言した申請状態（補正送信済・補正依頼着など）は古い情報です。\n"
        ctx += f"※申請状態を答える際は必ず以下のデータの値のみを使用し、会話履歴の値は使用しないこと。\n\n"
